...
(async()=>{
  try{
    ...
    approved=true;await guest.eval('pollPairing()');await until(()=>guest.eval('guestReady&&lastStateAt>0&&!edgeCountBaselinePending'),'signed ready and EDGE baseline');assert.equal(guest.document.querySelector('#guest-welcome').textContent,'WELCOME, SOPHIE');assert.equal(guest.eval('celebrationSerial'),0);assert.equal(guest.document.querySelector('#edge-celebration').hidden,true);
    ...
    const firstMessage=guest.document.querySelector('#celebration-message').textContent;assert.ok(firstCelebrations.has(firstMessage));
    ...
    const refreshed=page('controller.html','#ABCD',stored);await until(()=>refreshed.eval('guestReady&&lastStateAt>0&&!edgeCountBaselinePending'),'refresh');assert.equal(refreshed.document.querySelector('#pairing').hidden,true);await until(()=>refreshed.document.querySelector('#edge-count').textContent==='EDGE used: 2 times','refresh count');assert.equal(refreshed.eval('celebrationSerial'),0);assert.equal(refreshed.document.querySelector('#edge-celebration').hidden,true);
    await refreshed.eval('guestReady=false;channel.state="closed"');await host.eval('edgeUseCount=3');await refreshed.eval('recoverGuestSession()');await until(()=>refreshed.eval('guestReady&&!edgeCountBaselinePending&&edgeCount===3'),'reconnect restored newer count');assert.equal(refreshed.eval('celebrationSerial'),0);assert.equal(refreshed.document.querySelector('#edge-celebration').hidden,true);
    ...
    assert.deepEqual(errors,[]);console.log('PASS: verified EDGE celebration; silent refresh/reconnect baselines; replay and forgery rejection; local-only celebration preview; host/guest UI; fixed/HOLD controls; pairing; EDGE/repeat; dead-man; recovery; max14; visitor gate.');
