...
(async()=>{
  try{
    ...
    assert.equal(preview.document.querySelector('#tease-panel').dataset.stage,'1');click(preview,'[data-preview-stage="2"]');assert.equal(preview.document.querySelector('#tease-panel').dataset.stage,'2');assert.equal(preview.document.querySelector('#tease-message').textContent,'You’re really getting to him… 😏');click(preview,'[data-preview-stage="3"]');assert.equal(preview.document.querySelector('#tease-panel').dataset.stage,'3');assert.equal(preview.document.querySelector('#tease-message').textContent,'He’s ready — your move. ❤️');
    ...
    assert.equal(host.eval('teaseStage'),1);assert.equal(host.eval('teaseStartedAt'),null);assert.equal(host.document.querySelector('[data-tease-stage="1"]').getAttribute('aria-pressed'),'true');
    ...
    const hapticsAfterStage=guest.__haptics.length,lastSignedTease=host.__broadcasts.filter(message=>message.event==='tease-state').at(-1).payload;await guest.eval('receiveTeaseState('+JSON.stringify(lastSignedTease)+')');assert.equal(guest.__haptics.length,hapticsAfterStage);await guest.eval('receiveTeaseState({message:JSON.stringify({stage:3,startedAt:null,seq:9999}),signature:"AAAA"})');assert.equal(guest.document.querySelector('#tease-panel').dataset.stage,'2');assert.equal(guest.__haptics.length,hapticsAfterStage);
    await host.eval('setTeaseStage(4);receiveControl({id:"rejected-before-timer",command:"70",challenge:"invalid"})');assert.equal(host.eval('teaseStage'),2);assert.equal(host.eval('teaseStartedAt'),null);assert.equal(host.__writes.length,stageWriteCount);
    ...
    const activeHoldId=host.eval('engine.active.id'),holdWriteCount=host.__writes.length;click(host,'[data-tease-stage="2"]');await until(()=>guest.eval('teaseStage===2'),'stage during hold');assert.equal(host.eval('engine.active.id'),activeHoldId);assert.equal(host.eval('engine.active.command'),'max-hold');assert.equal(host.__writes.length,holdWriteCount);assert.equal(host.__writes.at(-1),'Vibrate:14;');
    ...
    assert.deepEqual(errors,[]);console.log('PASS: signed tease stages and spoof rejection; host-authoritative timer and silent recovery; acknowledgement-only haptics; isolated UI preview; EDGE celebration; fixed/HOLD controls; pairing; dead-man; recovery; max14; visitor gate.');
