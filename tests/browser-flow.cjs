...
(async()=>{
  ...
  try{
    ...
    assert.deepEqual(await host.locator('[data-tease-stage]').allTextContents(),['Relaxed','Frustration building','OMG','PLEASE']);await preview.locator('[data-preview-stage="2"]').click();assert.equal(await preview.locator('#tease-message').textContent(),'😏 BUILDING');await preview.locator('[data-preview-stage="3"]').click();assert.equal(await preview.locator('#tease-message').textContent(),'🔥 DESPERATE');await preview.locator('[data-preview-stage="4"]').click();assert.equal(await preview.locator('#tease-message').textContent(),'❤️ PLEASE');assert.equal(await preview.locator('#tease-copy').textContent(),'Okay… really. PLEASE.');await preview.locator('#preview-timer').click();assert.match(await preview.locator('#tease-timer').textContent(),/^Teasing for 18:4[12]$/);await preview.locator('#preview-haptic').click();assert.equal(await preview.locator('#haptic-state').textContent(),'TEASE TAP');
    ...
    assert.deepEqual(errors,[]);console.log('PASS: four signed tease stages, exact copy, timer recovery, acknowledgement haptics, isolated preview, pairing, signed EDGE feedback, dead-man, GATT recovery and max level.');
